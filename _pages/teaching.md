...
<!-- Optional: Accordion behaviour (only one semester open at a time).
<script>
  document.querySelectorAll('.teaching-semester').forEach((d) => {
    d.addEventListener('toggle', () => {
      if (!d.open) return;
      document.querySelectorAll('.teaching-semester').forEach((o) => {
        if (o !== d) o.open = false;
      });
    });
  });
